...
pub fn add_movie_review(
    ...
    let system_program = next_account_info(account_info_iter)?;
    let pda_counter_account = next_account_info(account_info_iter)?;
    ...
    if counter_pda != *pda_counter_account.key {
    ...
    invoke_signed(
        &system_instruction::create_account(
            ...
            pda_counter_account.key, // Address who we're creating the account for
    ...
    let mut counter_data =
        try_from_slice_unchecked::<MovieCommentCounter>(&pda_counter_account.data.borrow())
            .unwrap();
    ...
    msg!("comment count: {}", counter_data.counter);
    counter_data.serialize(&mut &mut pda_counter_account.data.borrow_mut()[..])?;
